...
[This document presents the results from Project Assignment 1 in the Coursera course Reproducible Research, written in a single R markdown document.]

Set the default of echo to be true throughout the document because the objective is to view the code and output:
```{r, echo=TRUE}
library(knitr)
opts_chunk$set
```
...
NOTE: Make sure you have already downloaded the activity.csv and saved it in your working directory. 
...
Review your data with "str()": 

```{r, echo=TRUE}
str(data)
```
Review your data with "head()":
...
2. Make a histogram using ggplot:
...
ggplot(steps_full, aes(x = steps)) +
  geom_histogram(fill = "gold", binwidth = 1000) +
